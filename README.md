...
- ```axes_linewidth```: Set the linewidth of axes. Default is ```1```. If a dictionary with keys $\in$ ```right```, ```left```, ```top```, ```bottom``` and with values $\in$ ```ìnt```, ```float``` is provided, only those sides, whose keys are given will be changed.
